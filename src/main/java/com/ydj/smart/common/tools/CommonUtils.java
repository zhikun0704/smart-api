...
public class CommonUtils
{
    ...
    public static boolean isEmail(String email)
    {
        ...
        return isEmptyString(email) ? false : PatternUtils.regex("^[\\w_-]+([\\.\\w_-]*[\\w_-]+)?@[\\w-]+\\.[a-zA-Z]+(\\.[a-zA-Z]+)?$", email, true);
    }
    ...
    public static boolean isMobile(String mobile)
    {
        ...
        return isEmptyString(mobile) ? false : PatternUtils.regex("^(\\+86(\\s)?)?0?1(3|4|5|8)\\d{9}$", mobile, true);
    ...
    public static String getPhoneNumber(String phoneNumber, boolean mobileOnly)
    {
        ...
        String ret = PatternUtils.parseStr(phoneNumber.replaceAll("\\s*", ""), "0?(1(3|4|5|8)\\d{9})", 1);
        
        return isMobile(ret) ? phoneNumber.startsWith("0") ? phoneNumber.replaceFirst("0", "") : phoneNumber : mobileOnly ? "" : ret;
    ...
    public static String subString(String str, int len)
    {
        ...
        if (str.length() < len)
            throw new IllegalArgumentException("The input string's length must>" + len);
    ...
    public static String intercept(String src, int length)
    {
        ...
        return lng < length ? src : src.substring(0, (int) ((length - 6) * len / lng)) + "…";
    }
    ...
    public static void writeFile(String file, String content, String charset) throws IOException
    {
        ...
        if (isEmptyString(content))
            throw new IOException("Can't write to file for the empty input param content");
    ...
    public static String encodePassword(String pswd)
    {
        ...
        if (pswd == null || pswd.length() < 6 || pswd.length() > 32)
            ...
            throw new IllegalArgumentException("Incorrect password! The password must not empty and it's length must between 6 and 32 bits long.");
    ...
    public static boolean isPhone(String phone)
    {
        ...
        return PatternUtils.regex("(?s)\\+?(\\s*(\\(\\d+\\)|\\d+)\\s*)*(\\d-?)+\\d+", phone, true);
