...
public class CookieUtils
{
    ...
    public static Cookie newCookie(String name, String value, boolean save_forever)
    {
    ...
    public static String getCookieValue(HttpServletRequest request, String cookieName)
    {
        ...
        for (int i = 0; i < cookies.length; i++)
        {
            if ((cookieName).equalsIgnoreCase(cookies[i].getName()))
            {
                try
                {
                    return URLDecoder.decode(new String(cookies[i].getValue().getBytes()), "utf-8");
                }
                catch (Exception e)
                {
    ...
    public static String toString(Cookie cookie)
    {
        ...
        sb.append("; domain=").append(COOKIE_DOMAIN).append("; path=/; ");
        if (cookie.getMaxAge() != -1)
            sb.append("expires=").append(GMTFormat.format(System.currentTimeMillis() + cookie.getMaxAge())).append("; ");
